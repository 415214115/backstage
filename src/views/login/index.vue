	...
	<div class="login-wrap">
		<el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
			...
			<el-form-item prop="phone">
				<el-input type="text" v-model="ruleForm.phone" auto-complete="off" maxlength="11" :validate-event="false" placeholder="账号"></el-input>
			</el-form-item>
			<el-form-item prop="password">
				<el-input type="password" v-model="ruleForm.password" auto-complete="off" :validate-event="false" placeholder="密码"></el-input>
			</el-form-item>
			<el-row>
				<el-col :span="16">
					<el-form-item prop="verifyCode">
						<el-input type="text" v-model="ruleForm.verifyCode" auto-complete="off" :validate-event="false" placeholder="图形验证码" @keyup.enter.native="submitForm('ruleForm')"></el-input>
					</el-form-item>
				</el-col>
				<el-col :span="8" class="code-box">
					<img :src="codeImg" alt="" class="codeimg" title="刷新验证码" @click="getcode()">
				</el-col>
			</el-row>
...
<script type="text/ecmascript-6">
	export default {
		...
		data() {
			...
			var loginCode = (rule, value, callback) => {
				if (!value) {
			...
			return {
				...
				rules: {
					...
					verifyCode: [
						{required: true,validator: loginCode}
					]
				}
			}
		},
		...
		methods: {
			// 获取图形验证码
			getcode() {
				this.$request.get(this.$api.loginCode).then( res => {
					// let url = 'data:image/png;base64,' + btoa(new Uint8Array(res).reduce((data, byte) => data + String.fromCharCode(
					// 		byte), ''))
					let url = 'data:image/png;base64,' + res.data.verifyCodeImg
					this.codeImg = url
					this.codeNum = res.data.verifyCode
				})
			},
			//获取info列表
			submitForm(formName) {
				this.$refs[formName].validate( (valid)  => {
					...
					if (valid) {
						// 注释
						if(this.ruleForm.verifyCode.toLowerCase() == this.codeNum){
							this.$request.postForm(this.$api.login,this.ruleForm).then( res => {
								if(res.code == 200){
